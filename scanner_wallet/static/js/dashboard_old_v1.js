...
function exportCSV() {
    ...
    csvContent += 'Signature,Type,Amount,Fee,Token,Wallet\n';

    transactions.forEach(tx => {
        const sig = tx.getAttribute('data-signature');
        const type = tx.querySelector('.transaction-type-badge')?.textContent.trim() || '';
        const amount = tx.querySelector('.amount-value')?.textContent.trim() || '';
        const fee = tx.querySelector('.fee-value')?.textContent.trim().replace('Frais: ', '') || '';
        const token = tx.querySelector('.token-symbol')?.textContent.trim() || 'SOL';
        const wallet = tx.querySelector('.wallet-indicator')?.textContent.trim() || '';
        csvContent += `${sig},${type},${amount},${fee},${token},${wallet}\n`;
    });
