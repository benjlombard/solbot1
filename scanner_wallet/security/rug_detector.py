...
def signal_handler(signum, frame):
    ...
    if logger:
        ...
        print("🛑 Signal d'arrêt reçu - Arrêt en cours...")
    running = False
...
class RugPullDetector:
    ...
    def __init__(self, db_path: str):
        ...
        self.logger = get_logger('rug_detector')
    ...
    def connect(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(f"✅ Connexion établie avec {self.db_path}")
                return True
            except Exception as e:
                self.logger.error(f"❌ Tentative {attempt + 1}/{max_retries} - Erreur connexion DB: {e}")
                if attempt < max_retries - 1:
                    self.logger.info(f"🔄 Nouvelle tentative dans {retry_delay}s...")
                    time.sleep(retry_delay)
                else:
                    self.logger.error("💥 Impossible de se connecter après plusieurs tentatives")
                    return False
    ...
    def reconnect_if_needed(self):
        ...
        try:
            ...
            self.logger.warning("🔄 Connexion perdue - Reconnexion...")
            self.disconnect()
            return self.connect()
    ...
    def get_suspect_tokens(self) -> Set[str]:
        ...
        try:
            ...
            self.logger.info(f"🔍 Requête 1 - Tokens suspects trouvés: {len(suspects)}")
            return suspects
        except Exception as e:
            self.logger.error(f"❌ Erreur requête suspects: {e}")
            return set()
    ...
    def get_confirmed_rugs(self) -> Set[str]:
        ...
        try:
            ...
            self.logger.info(f"🚨 Requête 2 - Rug pulls confirmés: {len(confirmed)}")
            return confirmed
        except Exception as e:
            self.logger.error(f"❌ Erreur requête confirmés: {e}")
            return set()
    ...
    def analyze_token_sets(self, suspects: Set[str], confirmed: Set[str]) -> Tuple[Set[str], Set[str], Set[str]]:
        ...
        if to_update:
            self.logger.info("=" * 50)
            self.logger.info("🎯 NOUVEAUX RUG PULLS DÉTECTÉS:")
            self.logger.info(f"   📌 Tokens à mettre à jour: {len(to_update)}")
            self.logger.info(f"   ⚠️ Tokens seulement suspects: {len(only_suspects)}")
            self.logger.info(f"   🤔 Tokens seulement confirmés: {len(only_confirmed)}")
            self.logger.info("=" * 50)
    ...
    def update_tokens_table(self, token_addresses: Set[str]) -> int:
        ...
        self.logger.info(f"📝 ÉTAPE 1: Mise à jour table tokens pour {len(token_addresses)} tokens")
        ...
        for token_address in token_addresses:
            try:
                ...
                if cursor.rowcount > 0:
                    ...
                    self.logger.info(f"   ✅ {token_address[:12]}... mis à jour (rug: {rug_date})")
                else:
                    self.logger.warning(f"   ⚠️ {token_address[:12]}... non trouvé dans table tokens")
                    
            except Exception as e:
                self.logger.error(f"   ❌ Erreur mise à jour {token_address[:12]}...: {e}")
        
        self.connection.commit()
        self.logger.info(f"💾 ÉTAPE 1 terminée: {success_count}/{len(token_addresses)} tokens mis à jour")
    ...
    def update_tokens_history_table(self, token_addresses: Set[str]) -> int:
        ...
        self.logger.info(f"📊 ÉTAPE 2: Mise à jour table tokens_history pour {len(token_addresses)} tokens")
        ...
        for token_address in token_addresses:
            try:
                ...
                if rows_updated > 0:
                    self.logger.info(f"   ✅ {token_address[:12]}... - {rows_updated} snapshots mis à jour")
                else:
                    self.logger.warning(f"   📭 {token_address[:12]}... - aucun snapshot mis à jour")
                    
            except Exception as e:
                self.logger.error(f"   ❌ Erreur mise à jour historique {token_address[:12]}...: {e}")
        
        self.connection.commit()
        self.logger.info(f"💾 ÉTAPE 2 terminée: {total_updated} snapshots mis à jour au total")
    ...
    def log_detection_summary(self, to_update: Set[str], tokens_updated: int, history_updated: int):
        ...
        if to_update:
            ...
            self.logger.info("🎯 NOUVEAUX RUG PULLS TRAITÉS:")
            self.logger.info(f"   💀 {tokens_updated} tokens marqués comme ruggés")
            self.logger.info(f"   📊 {history_updated} snapshots mis à jour")
            
            # Afficher quelques exemples
            for token in list(to_update)[:3]:
                info = self.get_token_info(token)
                symbol = info.get('symbol', 'N/A')
                self.logger.info(f"   📄 {token[:12]}... ({symbol})")
    ...
    def run_detection_cycle(self) -> bool:
        ...
        try:
            ...
            if to_update:
                self.logger.info("🚀 Nouveaux rug pulls détectés - Mise à jour en cours")
                ...
                self.logger.info(f"✨ Cycle {self.stats['total_cycles']} - Aucun nouveau rug pull détecté")
            
            self.last_check_time = datetime.now()
            return True
            
        except Exception as e:
            self.logger.error(f"❌ Erreur pendant le cycle de détection: {e}")
            return False
    ...
    def print_status_report(self):
        ...
        try:
            ...
            self.logger.info("=" * 60)
            self.logger.info("📊 RAPPORT DE STATUT DU SYSTÈME")
            self.logger.info("=" * 60)
            self.logger.info(f"⏰ Démarrage: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
            self.logger.info(f"⏱️ Uptime: {str(uptime).split('.')[0]}")
            self.logger.info(f"🔄 Cycles exécutés: {self.stats['total_cycles']}")
            self.logger.info(f"💾 Total tokens: {total_tokens}")
            self.logger.info(f"💀 Tokens ruggés: {total_rugged}")
            self.logger.info(f"🎯 Tokens ruggés détectés: {self.stats['total_tokens_updated']}")
            self.logger.info(f"📊 Snapshots mis à jour: {self.stats['total_history_updated']}")
            
            if self.stats['last_detection_time']:
                last_detection = self.stats['last_detection_time'].strftime('%Y-%m-%d %H:%M:%S')
                self.logger.info(f"🔍 Dernière détection: {last_detection}")
            
            if self.last_check_time:
                last_check = self.last_check_time.strftime('%H:%M:%S')
                self.logger.info(f"✅ Dernière vérification: {last_check}")
...
def main():
    ...
    try:
        ...
        rug_log_file = os.getenv('RUG_DETECTOR_LOG_FILE', 'rug_detector.log')
        rug_log_level = os.getenv('RUG_DETECTOR_LOG_LEVEL', config.logging.level.value)
        rug_log_max_size = int(os.getenv('RUG_DETECTOR_LOG_MAX_SIZE_MB', '50'))
        rug_log_backup_count = int(os.getenv('RUG_DETECTOR_LOG_BACKUP_COUNT', '10'))
        
        # Créer le logger spécialisé avec fichier dédié
        from core.logger import SolanaWalletLogger
        rug_logger = SolanaWalletLogger(
            log_level=rug_log_level,
            log_file=str(Path(config.logging.base_dir) / rug_log_file),
            console_output=config.logging.console_output,
            json_output=config.logging.json_output,
            max_file_size=rug_log_max_size * 1024 * 1024,
            backup_count=rug_log_backup_count,
            max_age_days=config.logging.max_age_days,
            force_reconfigure=True
        )
        
        logger = rug_logger.get_logger('rug_detector.main')
        
        # Paramètres de configuration
        db_path = config.database.get_full_path()
        check_interval = int(os.getenv('RUG_DETECTOR_CHECK_INTERVAL', '30'))  # secondes
        status_report_interval = int(os.getenv('RUG_DETECTOR_STATUS_INTERVAL', '300'))  # 5 minutes
    ...
    logger.info("🚀 DÉTECTION RUG PULLS - MODE CONTINU")
    logger.info("=" * 60)
    logger.info(f"⚙️ Intervalle vérification: {check_interval}s")
    logger.info(f"📊 Rapport de statut: {status_report_interval}s")
    logger.info(f"💾 Base de données: {db_path}")
    logger.info(f"📝 Log principal: {config.logging.get_full_path()}")
    logger.info(f"🎯 Log rug detector: {Path(config.logging.base_dir) / rug_log_file}")
    logger.info(f"📋 Niveau de log: {rug_log_level}")
    logger.info(f"📦 Taille max log: {rug_log_max_size}MB")
    logger.info(f"🔄 Fichiers backup: {rug_log_backup_count}")
    logger.info(f"ℹ️ Utilisez Ctrl+C pour arrêter proprement")
    logger.info("=" * 60)
    ...
    try:
        ...
        while running:
            ...
            if not success:
                logger.error("💥 Échec du cycle de détection - Tentative suivante dans 30s")
            ...
            logger.info(f"🔄 Cycle {cycle_count} terminé en {cycle_duration:.1f}s - Attente {wait_time:.1f}s")
        ...
        logger.info(f"⏱️ Uptime total: {str(uptime).split('.')[0]}")
        logger.info(f"🎯 Tokens ruggés détectés: {detector.stats['total_tokens_updated']}")
        logger.info("✅ Arrêt propre du script")
        logger.info("=" * 60)
