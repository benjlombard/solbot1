...
def main():
    st.title("📜 Analyse Historique des Tokens")
    st.markdown("*Explorez l'évolution des métriques clés pour chaque token au fil du temps.*")
    ...
    if 'df_history' not in st.session_state:
        with st.spinner("Chargement des données historiques..."):
            ...
            if not df.empty:
                ...
                st.error("❌ Impossible de charger les données historiques.")
                return
    
    df = st.session_state.df_history
    
    # Sélection du token
    st.sidebar.subheader("Sélection du Token")
    token_list = get_token_list(df)
    
    if not token_list:
        st.warning("Aucun token avec historique trouvé.")
        return
        
    selected_token_info = st.sidebar.selectbox(
        "Sélectionnez un token à analyser",
        token_list,
    ...
    if selected_token_info:
        ...
        st.header(f"Analyse de: {selected_token_info[0]}")
        st.code(token_address)
        ...
        if not df_token.empty:
            ...
            # Données brutes
            with st.expander("📋 Données Historiques Détaillées"):
                st.dataframe(df_token.drop(['id', 'previous_snapshot_id'], axis=1), use_container_width=True)
    else:
        st.info("Sélectionnez un token dans la barre latérale pour commencer.")
