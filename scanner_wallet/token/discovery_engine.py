...
try:
    ...
    def get_logger(name=None):
        logger = logging.getLogger(name or 'discovery_engine')
    ...
    def validate_wallet_address(addr):
        logging.getLogger('discovery_engine').warning("Using fallback address validator for %s", addr)
        return bool(addr and len(addr) == 44)
    def validate_token_mint(mint):
        logging.getLogger('discovery_engine').warning("Using fallback mint validator for %s", mint)
