        ...
        try:
            with self.db.get_connection_context() as conn:
                ...
                """, (
                    ...
                    snapshot_data.get('liquidity_mc_ratio', 0.0), 
                    snapshot_data.get('volume_mc_ratio', 0.0),
                    snapshot_data.get('price_volatility_1h', 0.0),
                    snapshot_data['volume_5m'], snapshot_data['volume_1h'], 
                    snapshot_data['volume_6h'], snapshot_data['volume_24h'],
                    snapshot_data['price_change_5m'], snapshot_data['price_change_1h'], 
                    snapshot_data['price_change_6h'], snapshot_data['price_change_24h'],
                    ...
                    scores['viability_score'], scores['risk_score'], scores['momentum_score'],
                    snapshot_data['rug_risk_score'], snapshot_data['rug_raw_score'],
                    snapshot_data['is_rugged'], snapshot_data['risk_count'],
                    snapshot_data.get('creator_address'), snapshot_data['symbol'], 
                    snapshot_data['name'], snapshot_data['decimals'], 
                    snapshot_data.get('logo_uri'), snapshot_data.get('is_verified', False), 
                    snapshot_data.get('metadata_source'),
                    current_timestamp, previous_snapshot_id,
                    deltas['price_delta_usd'], deltas['market_cap_delta'], 
                    deltas['volume_24h_delta'], deltas['holder_count_delta'],
                ...
                self.logger.debug(
                    ...
                    f"(V:{scores['viability_score']:.1f}, R:{scores['risk_score']:.1f}, "
                    f"M:{scores['momentum_score']:.1f})"
                )
                return True
