...
class QueueRepository:
    ...
    @db_retry(max_retries=3, delay=0.3)
    def get_pending_tokens(self, batch_size: int) -> List[str]:
        """Get a batch of pending tokens from the queue and mark as processing"""
        with self.db.get_connection_context() as conn:
            cursor = conn.cursor()
    ...
    @db_retry(max_retries=3, delay=0.3)
    def update_token_status(self, token_address: str, success: bool, error_message: Optional[str] = None):
    ...
    @db_retry(max_retries=3, delay=0.3)
    def add_tokens_to_queue(self, token_addresses: List[str]) -> int:
        ...
        with self.db.get_connection_context() as conn:
            ...
            data = [(addr, 'pending') for addr in token_addresses]
            cursor.executemany("""
                INSERT OR IGNORE INTO token_processing_queue (token_address, status, created_at)
                VALUES (?, ?, CURRENT_TIMESTAMP)
            """, data)
    ...
    def get_queue_stats(self) -> Dict:
        ...
        with self.db.get_connection_context() as conn:
            ...
            cursor.execute("""
                SELECT AVG(
                    CASE WHEN completed_at IS NOT NULL AND processing_started_at IS NOT NULL
            ...
            result = cursor.fetchone()
            stats['avg_processing_time_seconds'] = result[0] if result[0] else 0
