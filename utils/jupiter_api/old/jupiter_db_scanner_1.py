...
class TokenDatabaseScanner:
    ...
    def setup_database(self):
        ...
        CREATE TABLE IF NOT EXISTS tokens (
            ...
            dexes TEXT,
            price_impact REAL,
            quality_score REAL,  -- Changé en REAL pour scores décimaux
            ...
            tags TEXT,
            metadata TEXT,
            tx_count_24h INTEGER,  -- Nouveau champ
            has_verified_contract BOOLEAN DEFAULT 0,
            has_rug_check BOOLEAN DEFAULT 0,  -- Nouveau
            rug_score REAL,                  -- Nouveau
            issues TEXT                      -- Nouveau
        )
        ''')
    ...
    def get_alternative_token_data(self, token_address: str) -> Dict:
        ...
        sources = [
            ...
            ('birdeye', self.get_birdeye_data),  # Votre méthode existante
            ('jupiter_stats', self.get_jupiter_stats_data),  # Votre méthode existante
        ...
        if not combined_data['has_alternative_data']:
            ...
            for source_name, source_func in sources[:3]:  # Essayer les 3 premières sources
                try:
                    data = source_func(token_address)
                    # Merge les données utiles même si incomplètes
    ...
    def calculate_quality_score(self, token_data: Dict) -> float:
        """
        Calculer un score de qualité avancé pour un token basé sur plusieurs métriques.
        Score final entre 0 et 100, combinant liquidité, volume, âge, volatilité, risque, activité et aspects techniques.
        
        Args:
            token_data (Dict): Données du token (issues de Jupiter, DexScreener, etc.)
        
        Returns:
            float: Score de qualité entre 0 et 100
        """
        score_components = {
            'liquidity': 0.0,
            'volume': 0.0,
        ...
        weights = {
            'liquidity': 0.25,  # Importance de la liquidité pour le trading
            'volume': 0.20,    # Volume indique l'intérêt du marché
            'age': 0.15,       # Priorité aux tokens récents (< 24h)
            'volatility': 0.10, # Pénaliser forte volatilité
            'risk': 0.15,      # Importance des signaux de risque
            'activity': 0.10,  # Activité (transactions) indique l'engagement
            'technical': 0.05  # Aspects techniques (logo, tags, etc.)
        }
        
        # Helper function pour valider les nombres
        ...
        if liquidity_usd:
            # Normaliser : 100k USD = 1.0, linéaire jusqu'à 1M USD
            score_components['liquidity'] = min(liquidity_usd / 100000, 1.0)
            if liquidity_usd < 5000:  # Pénaliser faible liquidité
        ...
        if volume_24h and market_cap:
            ...
            # Ratio volume/mcap < 5 est sain, au-delà suspect (wash trading)
            score_components['volume'] = min(volume_ratio / 5, 1.0)
            if volume_ratio > 10:  # Pénaliser potentiel wash trading
        ...
        if age_hours != float('inf'):
            if 1 <= age_hours <= 24:
                score_components['age'] = 1.0  # Optimal pour tokens récents
            elif age_hours < 1:
                score_components['age'] = 0.5  # Trop jeune, risque élevé
            else:
                # Pénaliser les tokens plus vieux (> 1 semaine)
                score_components['age'] = max(0, 1 - (age_hours - 24) / 168)
        
        # 4. Volatilité
        price_change_24h = safe_float(token_data.get('price_change_24h'))
        if price_change_24h != 0:  # 0 = pas de données
            price_change_24h = abs(price_change_24h)
            score_components['volatility'] = max(0, 1 - price_change_24h / 50)
        else:
            score_components['volatility'] = 0.5  # Score neutre
        
         # 5. Risque (intégration de RugCheck)
        if token_data.get('has_rug_check', False):
            rug_score = safe_float(token_data.get('rug_score'), 50)  # 50 = neutre
            score_components['risk'] = max(0, (100 - rug_score) / 100)
        else:
            # Logique par défaut si RugCheck n'est pas disponible
            ...
            if token_data.get('has_dexscreener_data', False):
                score_components['risk'] *= 0.9
        
        # 6. Activité
        route_count = safe_float(token_data.get('route_count'))
        # Simuler activité avec route_count (à remplacer par tx_count si disponible)
        ...
        token_data['liquidity_score'] = round(score_components['liquidity'], 2)
        token_data['volume_score'] = round(score_components['volume'], 2)
        token_data['age_score'] = round(score_components['age'], 2)
        token_data['volatility_score'] = round(score_components['volatility'], 2)
        token_data['risk_score'] = round(score_components['risk'], 2)
        token_data['activity_score'] = round(score_components['activity'], 2)
        token_data['technical_score'] = round(score_components['technical'], 2)
    ...
    def get_orca_data(self, token_address: str) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for pool in pools:
                    if token_address in [pool['tokenA']['mint'], pool['tokenB']['mint']]:
                        return {
                            'has_orca_data': True,
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    'volume_usd': token_data.get('volume_usd', {})
                }
    ...
    def get_helius_data(self, token_address: str) -> Dict:
        """Récupérer les données depuis Helius"""
        try:
            # Token metadata
            payload = {
                "jsonrpc": "2.0",
            ...
            response = self.session.post(
                "https://rpc.helius.xyz/?api-key=YOUR_API_KEY",  # Remplacer par votre clé
                json=payload,
                timeout=8
            )
    ...
    def get_token_data_with_priority(self, token_address: str):
        ...
        return {'has_data': False}  # Toujours retourner un dict

    def get_onchain_data(self, token_address: str):
        """Récupère les données directement de la blockchain"""
        from solana.rpc.api import Client
        solana_client = Client("https://api.mainnet-beta.solana.com")
        
        try:
            account_info = solana_client.get_account_info(token_address)
            # Parsing des données on-chain ici
            return {
    ...
    async def get_underlying_token(self, wrapped_address: str):
        """Trouve le token original pour les wrapped tokens"""
        try:
            # Essayez de trouver l'original via l'API Wormhole par exemple
            response = await self.session.get(
                f"https://api.wormholescan.io/api/v1/tokens?address={wrapped_address}"
            )
            if response.status_code == 200:
                data = await response.json()
                return data.get('nativeAddress')
        except Exception:
            return None

    def check_holders(self, token_address: str):
        """Vérifie le nombre de holders"""
        try:
            response = requests.get(
                f"https://api.solscan.io/token/holders?tokenAddress={token_address}&limit=1"
            )
    ...
    def enrich_token_data(self, token_address: str, base_data: Dict) -> Dict:
        ...
        if not dex_data.get('has_dexscreener_data'):
            try:
                ...
                if self.is_wrapped_token(token_data):
                    underlying = await self.get_underlying_token(token_address)
                    if underlying:
                        underlying_data = await self.get_dexscreener_data(underlying)
                ...
                if not token_data.get('has_alternative_data'):
                    alt_data = await self.get_token_data_with_priority(token_address)
                    if alt_data and isinstance(alt_data, dict):
                        token_data.update(alt_data)

                # 4c. Dernier recours: vérification on-chain
                if not token_data.get('has_alternative_data'):
                    onchain_data = await self.get_onchain_data(token_address)
                    if onchain_data:
        ...
        # 5. Calcul du score avec gestion des erreurs
        try:
        ...
        # 6. Validation finale des données
    ...
    async def check_onchain_activity(self, token_address: str):
        """Vérifie l'activité on-chain quand aucune donnée de marché n'est disponible"""
        ...
        try:
            ...
            return len(signatures.value) > 0
        except Exception as e:
            self.logger.error(f"On-chain check failed: {e}")
            return False
        finally:
            await client.close()
    ...
    def get_birdeye_data(self, token_address: str) -> Dict:
        """Votre code Birdeye existant"""
        # 1. Essayer Birdeye API
        try:
            response = self.session.get(
            ...
            return {'has_birdeye_data': False}  # ← Toujours retourner un dict

    def get_jupiter_stats_data(self, token_address: str) -> Dict:
        """Votre code Jupiter Stats existant"""  
        try:
            response = self.session.get(
        ...
        return {'has_alternative_data': False}
    ...
    def display_token_info(self, token_data: Dict):
        ...
        print(f"Score Breakdown: Liquidity={token_data.get('liquidity_score', 0):.2f}, "
    ...
    def check_rug_pull_risk(self, token_address: str) -> Dict:
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    'has_rug_check': True,
                    'rug_score': rug_score,  # Utiliser la variable validée
